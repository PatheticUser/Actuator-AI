...
RUN apt-get update && \
    ...
    apt-get update && \
    apt-get install -y --no-install-recommends nodejs tesseract-ocr libtesseract-dev && \
...
CMD ["sh", "-c", "uv run uvicorn backend.main:app --host 0.0.0.0 --port ${PORT:-8000} --workers 2"]
